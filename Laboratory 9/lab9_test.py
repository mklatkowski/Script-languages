...
import datetime
import ipaddress
from sshLogJournal_typed import to_SSHLogEntry
from sshLogJournal_typed import SSHLogJournal
from sshLogEntry_typed import SSHFailedPassword, SSHAcceptedPassword, SSHAError, SSHAOther
...
def test_to_SSHLogEntry_time(log, time):
    assert to_SSHLogEntry(log).timestamp == time
...
@pytest.mark.parametrize(
    "log_list, log, log_type", [
        (
            SSHLogJournal(),
            "Dec 10 07:11:42 LabSZ sshd[24224]: input_userauth_request: invalid user chen [preauth]",
            type(SSHAOther("Dec 10 07:11:42 LabSZ sshd[24224]: input_userauth_request: invalid user chen [preauth]", datetime.datetime.strptime("Dec 10 07:11:42", '%b %d %H:%M:%S'), "LabSZ", "input_userauth_request: invalid user chen [preauth]", "24224"))
        ),
        (
            SSHLogJournal(),
            "Dec 10 06:55:48 LabSZ sshd[24200]: Failed password for invalid user webmaster from 173.234.31.186 port 38926 ssh2",
            type(SSHFailedPassword("Dec 10 06:55:48 LabSZ sshd[24200]: Failed password for invalid user webmaster from 173.234.31.186 port 38926 ssh2", datetime.datetime.strptime("Dec 10 06:55:48", '%b %d %H:%M:%S'), "LabSZ", "Failed password for invalid user webmaster from 173.234.31.186 port 38926 ssh2", "24200"))
        ),
        (
            SSHLogJournal(),
            "Dec 10 09:32:20 LabSZ sshd[24680]: Accepted password for fztu from 119.137.62.142 port 49116 ssh2",
            type(SSHAcceptedPassword("Dec 10 09:32:20 LabSZ sshd[24680]: Accepted password for fztu from 119.137.62.142 port 49116 ssh2", datetime.datetime.strptime("Dec 10 09:32:20", '%b %d %H:%M:%S'), "LabSZ", "Accepted password for fztu from 119.137.62.142 port 49116 ssh2", "24680"))
        ),
        (
            SSHLogJournal(),
            "Dec 10 11:03:44 LabSZ sshd[25455]: error: Received disconnect from 103.99.0.122: 14: No more user authentication",
            type(SSHAError("Dec 10 11:03:44 LabSZ sshd[25455]: error: Received disconnect from 103.99.0.122: 14: No more user authentication", datetime.datetime.strptime("Dec 10 11:03:44", '%b %d %H:%M:%S'), "LabSZ", "error: Received disconnect from 103.99.0.122: 14: No more user authentication", "25455"))
        )
    ])

def test_append(log_list, log, log_type):
    log_list.append(log)
    assert type(log_list[-1]) == log_type
